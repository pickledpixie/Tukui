...
Mod_AddonSkins:RegisterSkin("Quartz",function(Skin, skin, Layout, layout, config)
	...
	template.ApplySettings = function (self)
		self:ApplySettings_()
		self:SetWidth(TukuiDB.Scale(self.config.w) + TukuiDB.Scale(config.borderWidth) * 2)
		self:SetHeight(TukuiDB.Scale(self.config.h) + TukuiDB.Scale(config.borderWidth) * 2)
